...
export function TitleDetailModal({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const primaryEdition =
          commerceEditions?.find((e) =>
            (detail.sources ?? []).some(
              (s) => s.toLowerCase() === e.source.toLowerCase(),
            ),
          ) ?? commerceEditions?.[0];
        const res = await fetchPurchaseHints({
          ...
          asin: commerceAsin,
          isbn: commerceIsbn,
          store_editions: commerceEditions,
    ...
    commerceAsin,
    commerceIsbn,
    // Serialize editions so parent array identity churn does not refetch.
    commerceEditionsKey,
  ]);
