...
| `AdapterDatabaseSession.capabilities` | Typed control-plane call after `openSession`. Advertises SQL contract version, execution semantics, `schemaMigrations`, backup flags, and all limits. Host policy requires `schemaMigrations` and ignores `pragmaUserVersion` / `atomicSchemaBatch` / `timing`. The host must not invent these from the plugin id. |
