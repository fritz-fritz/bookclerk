...
accepts the operator token (`POST /api/auth/login` returns 403); the system
tray still opens a session via loopback `GET /api/auth/tray-handoff` (no token
in the query string; a 60-second ticket is minted by
`POST /api/auth/tray-handoff/prepare` with Bearer), and `Authorization:
Bearer` API access is unchanged. The handoff is refused unless the TCP peer is
loopback, `Host` is localhost/`127.0.0.1`/`::1`, and no `X-Forwarded-*` /
`Forwarded` headers are present — so a same-host reverse proxy cannot mint an
operator cookie on the public origin. The system tray **Copy operator token** menu copies to
the clipboard for **60 seconds**, then removes that exact token if it is still
present (never prints the value).
