...
`plugin = "postgres"` connects to a PostgreSQL **16+** server using the
sqlx-postgres driver inside SeaORM. Connect fails closed when
...
The guest is native-behind-workerd. The sibling `NetPolicy::Deny` jail
blocks ambient TCP; sqlx talks to a guest-local Unix socket that the SDK
splices through the inherited CONNECT mux (Linux/macOS). Host spawn
overlays TCP + loopback/private CIDRs implied by `[database.postgres].url`
onto the guest selected by `[database].plugin` (canonical PluginKey, or an
unambiguous kind token such as `postgres` / `pg`). The overlay is not
persisted as invented structural authority. A second install that reuses
`id = "postgres"` does not receive the operator URL when `[database].plugin`
names the other PluginKey. Windows LIKE uses the same sibling Deny plus the
inherited `handle:` mux for SDK HTTP; postgres Unix-socket mediation is
Unix-only. When `BOOKCLERK_SOCKET_PROXY` is set on Windows the guest
fail-closes rather than dialing ambient TCP (sqlx has no Unix-socket
rewrite there).
