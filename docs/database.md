...
**Schema migrations**: Fresh Postgres databases apply the frozen host pack
([`latest_schema_postgres`](../crates/bookclerk-library/src/migrations.rs),
canonical SQLite DDL lowered at the adapter edge in
[`schema_postgres.rs`](../crates/bookclerk-db-exec/src/schema_postgres.rs)
to `BIGSERIAL` / `BIGINT` / `BYTEA`) and record version `1` in
`schema_migrations` (checksum, app version, applied-at). SQLite also sets
`PRAGMA user_version`. A database newer than this binary **fails closed**;
see [schema versioning](adr/schema-versioning.md) and `bookclerk db`.
...
- Schema versions are **host-owned**. After `db.connect` and capability
  ...
  generic execute (each host schema version is one host-compiled
  `{ "batch": [...] }` on D1). A database newer than this binary fails
  closed (`bookclerk db downgrade` / restore a snapshot).
...
### Frozen host schema (version 1)

[`migrations.rs`](../crates/bookclerk-library/src/migrations.rs) exposes
`latest_schema_sqlite()` as the **frozen** version-1 pack
(`SCHEMA_VERSION = 1`). Postgres receives that pack lowered mechanically
([`schema_postgres.rs`](../crates/bookclerk-db-exec/src/schema_postgres.rs)).
There is no live incremental chain: historical V2–V29 fragments are
`#[cfg(test)]` archaeology only. After freeze, land new DDL in
`UNRELEASED_SQL` until a release cut promotes it to the next
`HostMigrationStep` with a checksum and `introduced_in`. See
[ADR: schema versioning](adr/schema-versioning.md).

Library open compares the database to this binary:

- **older DB** — snapshot under `$BOOKCLERK_FILES_DIR/snapshots/`, then apply remaining ups
- **same version** — verify `schema_migrations.checksum`; mismatch refuses
- **newer DB** — fail closed (never auto-downgrade). Use `bookclerk db downgrade`
  or restore a snapshot.

`bookclerk db version|snapshot|restore|migrate|downgrade` inspects and walks
schema without applying on connect. Schema 1 has `down: None` (irreversible).
