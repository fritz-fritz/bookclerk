...
fn main() {
    ...
    let Ok(output) = std::process::Command::new("rustc").arg("-vV").output() else {
        println!("cargo:rustc-env=LIBATION_RUSTC_RELEASE=unknown");
        println!("cargo:rustc-env=LIBATION_RUSTC_CHANNEL=unknown");
        return;
    };
    ...
    println!("cargo:rustc-env=LIBATION_RUSTC_RELEASE={release}");
    println!("cargo:rustc-env=LIBATION_RUSTC_CHANNEL={channel}");
    if !host.is_empty() {
        println!("cargo:rustc-env=LIBATION_RUSTC_HOST={host}");
    }
}
