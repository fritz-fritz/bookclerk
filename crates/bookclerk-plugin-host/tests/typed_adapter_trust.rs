...
use bookclerk_plugin_abi::{
    typecheck_execute_request_proofs, AdapterExecuteRequest, DbCapabilities, DbColumn,
    DbPlanStatementKind, DbResultSelection, DbRow, DbType, DbValue, ExecuteReply, ExecuteRequest,
    GuestReceiptPersist, PluginError as AbiPluginError, PluginErrorCode, StatementResult,
    TypedDbStatement,
};
...
async fn external_adapter_replays_named_atomic_after_commit() {
    ...
    let envelope = AdapterExecuteRequest::new(typed.clone(), GuestReceiptPersist::default())
        .with_proofs(proofs);
    let first = adapter
