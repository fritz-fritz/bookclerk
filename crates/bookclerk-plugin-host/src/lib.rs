...
pub use consent::{
    ...
    effective_max_processes, effective_memory_mib, format_cpu_cores, grant_covers, grant_has_binding,
    grant_within_ceiling, handshake_config_for_grant, host_cpu_cores_max, host_cpu_rate_max,
    host_logical_cpus, inject_workerd_grant_env, is_platform_plugin_id, network_compatible,
    percent_to_cores, require_binding, require_grant, spawn_grant, validate_approved_grant,
    validate_handshake_capabilities, PluginGrant, PluginGrantStore, GRANTS_FILE,
    KNOWN_HOST_BINDINGS, PLUGIN_JAIL_CPU_CORES_DEFAULT, PLUGIN_JAIL_CPU_RATE_DEFAULT,
