...
pub async fn guest_capabilities(
    ...
    Ok(match conn.get_database_backend() {
        ...
        DbBackend::Sqlite => DbCapabilities::advertised_sqlite(),
        other => panic!("BookclerkSQL does not support SeaORM backend {other:?}"),
    })
}
...
pub async fn guest_bootstrap(
    ...
    Ok(match conn.get_database_backend() {
        ...
        DbBackend::Sqlite => bookclerk_plugin_abi::DbBootstrap::sqlite(),
        other => panic!("BookclerkSQL does not support SeaORM backend {other:?}"),
    })
}
                ...
                let result = match stack_txn(&stack, &txn_id) {
                    Ok(txn) => {
                        let caps = match ConnectionTrait::get_database_backend(txn) {
                            ...
                            DbBackend::Sqlite => DbCapabilities::advertised_sqlite(),
                            other => {
                                panic!("BookclerkSQL does not support SeaORM backend {other:?}")
                            }
                        };
    ...
    <C as TransactionTrait>::Transaction: ConnectionTrait + StreamTrait + TransactionSession,
{
    ...
    let rows = match backend {
        ...
        other => {
            return Err(format!(
                "BookclerkSQL does not support SeaORM backend {other:?}"
            ));
        }
    };
    if rows.len() > fetch {
