//! Data-directory resolution (env, XDG, cwd fallback).
...
/// Environment variable for the Bookclerk data directory.
pub const BOOKCLERK_FILES_DIR_ENV: &str = "BOOKCLERK_FILES_DIR";

/// Override for the config file path (`BOOKCLERK_CONFIG`).
pub const BOOKCLERK_CONFIG_ENV: &str = "BOOKCLERK_CONFIG";

/// Resolved filesystem locations for Bookclerk data.
...
pub struct Paths {
    ...
    /// Reserved path (legacy). Logging goes to stderr + journald; Bookclerk does
...
/// Resolve the Bookclerk files directory.
///
/// Precedence:
/// 1. Explicit `--bookclerk-files` / caller-provided path
/// 2. `BOOKCLERK_FILES_DIR`
/// 3. XDG data dir: `$XDG_DATA_HOME/bookclerk` (or platform equivalent)
/// 4. Fallback: `./BookclerkFiles` (cwd-relative, Docker-friendly)
#[must_use]
pub fn resolve_files_dir(cli_override: Option<PathBuf>) -> PathBuf {
    resolve_files_dir_with(cli_override, env::var(BOOKCLERK_FILES_DIR_ENV).ok())
}
...
pub fn resolve_files_dir_with(
    ...
    if let Some(proj) = ProjectDirs::from("com", "Bookclerk", "bookclerk") {
        return proj.data_dir().to_path_buf();
    }

    PathBuf::from("BookclerkFiles")
}

/// Resolve config file path: `BOOKCLERK_CONFIG` → `{files_dir}/config.toml`.
#[must_use]
pub fn resolve_config_path(files_dir: &std::path::Path) -> PathBuf {
    if let Ok(path) = env::var(BOOKCLERK_CONFIG_ENV) {
        let trimmed = path.trim();
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed);
        }
...
mod tests {
    ...
    fn cli_override_wins() {
        let path = resolve_files_dir_with(
            Some(PathBuf::from("/tmp/custom-bookclerk")),
            Some("/var/lib/bookclerk".into()),
        );
        assert_eq!(path, PathBuf::from("/tmp/custom-bookclerk"));
    }

    #[test]
    fn env_var_used_when_no_cli_override() {
        let path = resolve_files_dir_with(None, Some("/var/lib/bookclerk".into()));
        assert_eq!(path, PathBuf::from("/var/lib/bookclerk"));
    }
