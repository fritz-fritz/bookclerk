...
use super::TrimRange;
use crate::crypto::{decrypt_cenc_sample_in_place, expand_cenc_iv, parse_aes128_hex};
...
fn parse_dash_file(path: &Path) -> Result<DashFileInfo> {
    ...
    let _ftyp = ftyp.ok_or_else(|| DecryptError::Mp4("DASH: missing ftyp".into()))?;
    let moov = moov.ok_or_else(|| DecryptError::Mp4("DASH: missing moov".into()))?;
    let first_moof = first_moof.ok_or_else(|| DecryptError::Mp4("DASH: missing moof".into()))?;
    let first_mdat = first_mdat.ok_or_else(|| DecryptError::Mp4("DASH: missing mdat".into()))?;
...
fn parse_dash_audio_track(file: &mut File, moov: &BoxHeader) -> Result<DashAudioMeta> {
    ...
    let track = result.ok_or_else(|| DecryptError::Mp4("DASH: no audio track".into()))?;
    Ok(DashAudioMeta {
...
fn parse_tenc_kid_from_entry(
    ...
    let schi = find_child(file, sinf.content_start(), sinf.end(), SCHI)?
        .ok_or_else(|| DecryptError::Mp4("sinf missing schi".into()))?;
    let tenc = find_child(file, schi.content_start(), schi.end(), TENC)?
        .ok_or_else(|| DecryptError::Mp4("schi missing tenc".into()))?;
...
fn parse_fragment(file: &mut File, moof: &BoxHeader, dash: &DashFileInfo) -> Result<FragmentInfo> {
    let traf = find_child(file, moof.content_start(), moof.end(), TRAF)?
        .ok_or_else(|| DecryptError::Mp4("moof missing traf".into()))?;
    let tfhd = find_child(file, traf.content_start(), traf.end(), TFHD)?
        .ok_or_else(|| DecryptError::Mp4("traf missing tfhd".into()))?;
    ...
    let trun = find_child(file, traf.content_start(), traf.end(), TRUN)?
        .ok_or_else(|| DecryptError::Mp4("traf missing trun".into()))?;
    ...
    for _ in 0..sample_count {
        let duration = if has_duration {
            ...
            default_duration.ok_or_else(|| {
                DecryptError::Mp4("trun missing sample durations and no tfhd/trex default".into())
            })?
        };
        let size = if has_size {
            ...
            default_size.ok_or_else(|| {
                DecryptError::Mp4("trun missing sample sizes and no tfhd/trex default".into())
            })?
        };
        if has_flags {
            let _ = read_u32(file)?;
...
fn parse_senc_ivs(
    ...
    if pos > senc.end() {
        return Err(DecryptError::Mp4("senc underflow".into()));
    }
...
fn normalize_cenc_iv(iv: &[u8]) -> Result<[u8; 16]> {
    match iv.len() {
        ...
        n => Err(DecryptError::Mp4(format!("unexpected CENC IV length {n}"))),
    }
}
...
fn patch_dash_moov(moov_bytes: &[u8]) -> Result<Vec<u8>> {
    ...
    let (stsd_start, stsd_end) = find_direct_child(&body, stbl_start, stbl_end, b"stsd")?
        .ok_or_else(|| DecryptError::Mp4("stbl missing stsd".into()))?;

    let entry_pos = stsd_start + 16;
    if entry_pos + 8 > stsd_end {
        return Err(DecryptError::Mp4("stsd truncated".into()));
    }
    let entry_size =
    ...
    if entry_end > stsd_end || entry_size < 36 {
        return Err(DecryptError::Mp4("invalid sample entry".into()));
    }
...
fn remove_moov_children_named(moov: &[u8], names: &[&[u8; 4]]) -> Result<Vec<u8>> {
    if moov.len() < 8 || &moov[4..8] != b"moov" {
        return Err(DecryptError::Mp4("expected moov box".into()));
    }
