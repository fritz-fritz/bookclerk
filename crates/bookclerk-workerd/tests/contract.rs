...
impl Destination for CountingDest {
    async fn head(
        &self,
        _key: &str,
    ) -> bookclerk_plugin_abi::Result<Option<bookclerk_plugin_abi::ObjectMetadata>> {
        Ok(None)
    }
    async fn list(
        &self,
        _options: ListOptions,
    ) -> bookclerk_plugin_abi::Result<bookclerk_plugin_abi::ListPage> {
        Ok(bookclerk_plugin_abi::ListPage::default())
    }
    async fn get(
        &self,
        key: &str,
        _range: Option<ByteRange>,
    ) -> bookclerk_plugin_abi::Result<bookclerk_plugin_abi::ReadResult> {
        let size: u64 = key
            .strip_prefix("pattern:")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        Ok(bookclerk_plugin_abi::ReadResult {
            meta: bookclerk_plugin_abi::ObjectMetadata {
                key: key.into(),
                size,
                ..Default::default()
            },
    ...
    async fn put(
    ...
    ) -> bookclerk_plugin_abi::Result<bookclerk_plugin_abi::PutResult> {
        let mut n = 0u64;
        ...
        *self.written.lock().expect("lock") = n;
        Ok(bookclerk_plugin_abi::PutResult {
            key: key.into(),
            bytes_written: n,
            ..Default::default()
        })
    ...
    async fn copy(
        ...
        _from: &str,
        _to: &str,
    ) -> bookclerk_plugin_abi::Result<bookclerk_plugin_abi::CopyResult> {
...
async fn workerd_stream_and_job_handler_contract() {
    let Some(workerd) = find_workerd() else {
        if skip_workerd_allowed() {
            eprintln!("skipping workerd contract (BOOKCLERK_SKIP_WORKERD=1)");
            return;
        }
        panic!(
            "pinned workerd binary missing; run `cargo ensure-workerd` / `cargo build-app --platform`. \
             Local native-only skip: BOOKCLERK_SKIP_WORKERD=1"
        );
    };
    ...
    local
        .run_until(async {
            ...
            assert_eq!(desc.id, "stream_fixture");
...
#[tokio::test(flavor = "current_thread")]
async fn workerd_separate_instances_do_not_share_objects() {
    let Some(workerd) = find_workerd() else {
        if skip_workerd_allowed() {
            eprintln!("skipping workerd isolation (BOOKCLERK_SKIP_WORKERD=1)");
            return;
        }
