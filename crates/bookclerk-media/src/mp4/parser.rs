...
pub fn parse_mp4(path: &Path) -> Result<Mp4File> {
    ...
    let ftyp = ftyp.ok_or_else(|| MediaError::Mp4("missing ftyp box".into()))?;
    let moov = moov.ok_or_else(|| MediaError::Mp4("missing moov box".into()))?;
    let mdat = mdat.ok_or_else(|| MediaError::Mp4("missing mdat box".into()))?;
    ...
    let mut ftyp_bytes = vec![
        ...
        usize::try_from(ftyp.size).map_err(|_| {
            MediaError::Mp4(format!("ftyp too large: {}", ftyp.size))
        })?
    ];
    ...
    let mut moov_bytes = vec![
        ...
        usize::try_from(moov.size).map_err(|_| {
            MediaError::Mp4(format!("moov too large: {}", moov.size))
        })?
    ];
...
fn parse_mvhd(file: &mut File, moov: &BoxHeader) -> Result<(u32, u64)> {
    let mvhd = find_child(file, moov.content_start(), moov.end(), super::boxutil::MVHD)?
        .ok_or_else(|| MediaError::Mp4("missing mvhd".into()))?;
...
fn parse_audio_track(file: &mut File, moov: &BoxHeader) -> Result<AudioTrack> {
    ...
    audio.ok_or_else(|| MediaError::Mp4("no audio track found in moov".into()))
}
...
fn try_parse_audio_trak(file: &mut File, trak: &BoxHeader) -> Result<Option<AudioTrack>> {
    ...
    let mdhd = find_child(file, mdia.content_start(), mdia.end(), MDHD)?
        .ok_or_else(|| MediaError::Mp4("audio track missing mdhd".into()))?;
    ...
    let minf = find_child(file, mdia.content_start(), mdia.end(), MINF)?
        .ok_or_else(|| MediaError::Mp4("audio track missing minf".into()))?;
    let stbl = find_child(file, minf.content_start(), minf.end(), STBL)?
        .ok_or_else(|| MediaError::Mp4("audio track missing stbl".into()))?;
...
fn parse_stsd(file: &mut File, stbl: &BoxHeader) -> Result<(SampleEntryKind, u64)> {
    let stsd = find_child(file, stbl.content_start(), stbl.end(), STSD)?
        .ok_or_else(|| MediaError::Mp4("missing stsd".into()))?;
    file.seek(SeekFrom::Start(stsd.content_start()))?;
    let (_version, _) = read_full_box_version_flags(file)?;
    let entry_count = read_u32(file)?;
    if entry_count == 0 {
        return Err(MediaError::Mp4("stsd has no sample entries".into()));
    }
...
fn parse_stts(file: &mut File, stbl: &BoxHeader) -> Result<Vec<(u32, u32)>> {
    let stts = find_child(file, stbl.content_start(), stbl.end(), STTS)?
        .ok_or_else(|| MediaError::Mp4("missing stts".into()))?;
...
fn parse_stsc(file: &mut File, stbl: &BoxHeader) -> Result<Vec<ChunkMapEntry>> {
    let stsc = find_child(file, stbl.content_start(), stbl.end(), STSC)?
        .ok_or_else(|| MediaError::Mp4("missing stsc".into()))?;
...
fn parse_stsz(file: &mut File, stbl: &BoxHeader) -> Result<Vec<u32>> {
    ...
    Err(MediaError::Mp4("missing stsz".into()))
}
...
fn parse_chunk_offsets(file: &mut File, stbl: &BoxHeader) -> Result<Vec<u64>> {
    ...
    Err(MediaError::Mp4("missing stco/co64".into()))
}
...
pub fn extract_mp4a_config(mp4: &Mp4File) -> Result<Mp4aConfig> {
    ...
    let type_rel = usize::try_from(
        mp4.audio
            ...
            .ok_or_else(|| MediaError::Mp4("sample entry outside moov".into()))?,
    )
    .map_err(|_| MediaError::Mp4("sample entry offset overflow".into()))?;
    if type_rel < 4 || type_rel + 4 > moov.len() {
        return Err(MediaError::Mp4("mp4a type offset invalid".into()));
    }
    ...
    if entry_end > moov.len() || entry_size < 36 {
        return Err(MediaError::Mp4("mp4a sample entry truncated".into()));
    }
    ...
    let esds = find_child_fourcc(moov, children_start, entry_end, b"esds")?
        .ok_or_else(|| MediaError::Mp4("mp4a missing esds".into()))?;
...
fn parse_asc_from_esds(esds_box: &[u8]) -> Result<Vec<u8>> {
    if esds_box.len() < 12 {
        return Err(MediaError::Mp4("esds too small".into()));
    }
    ...
    while i < esds_box.len() {
        ...
        let end = i
            ...
            .ok_or_else(|| MediaError::Mp4("esds descriptor truncated".into()))?;
        if tag == 0x05 {
        ...
        if tag == 0x03 {
            ...
            if end - i < 3 {
                return Err(MediaError::Mp4("ES_Descriptor truncated".into()));
            }
            ...
            if flags & 0x40 != 0 {
                if nest >= end {
                    return Err(MediaError::Mp4("ES_Descriptor URL truncated".into()));
                }
                let url_len = esds_box[nest] as usize;
                nest = nest
                    .checked_add(1 + url_len)
                    .ok_or_else(|| MediaError::Mp4("ES_Descriptor URL overflow".into()))?;
            }
            if flags & 0x20 != 0 {
                nest += 2; // OCR ES_ID
            }
...
fn find_desc_tag(buf: &[u8], want: u8) -> Result<Option<Vec<u8>>> {
    ...
    while i < buf.len() {
        ...
        let end = i
            .checked_add(len)
            .filter(|e| *e <= buf.len())
            .ok_or_else(|| MediaError::Mp4("descriptor truncated".into()))?;
        if tag == want {
...
fn read_expandable_len(buf: &[u8], mut i: usize) -> Result<(usize, usize)> {
    ...
    for _ in 0..4 {
        if i >= buf.len() {
            return Err(MediaError::Mp4("expandable length truncated".into()));
        }
        let b = buf[i];
        i += 1;
    ...
    Err(MediaError::Mp4("expandable length too long".into()))
}
