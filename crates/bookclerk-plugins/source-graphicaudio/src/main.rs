    ...
    PluginGuest::serve(|method, params| async move {
        match method.as_str() {
            ...
            methods::LOGIN => {
                ...
                let dto =
                    bookclerk_plugin_source_graphicaudio::guest_login_rpc(&access_url, &store_url, access, p)
                        .await
            ...
            methods::FETCH_TITLE => {
                ...
                let access_url = bookclerk_plugin_source_graphicaudio::resolve_access_base_url(&p.source_config);
                let store_url = bookclerk_plugin_source_graphicaudio::resolve_store_base_url(&p.source_config);
                let access = bookclerk_plugin_source_graphicaudio::resolve_access(&p.source_config);
                let bitrate = bookclerk_plugin_source_graphicaudio::resolve_bitrate(&p.source_config);
                let container = bookclerk_plugin_source_graphicaudio::resolve_container(&p.source_config);
