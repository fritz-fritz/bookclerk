//! Port of key cases from Bookclerk's `TemplatesTests.cs`.
//!
...
fn book() -> BookContext {
    BookContext {
        ...
        publisher: None,
        categories: vec![],
        bookclerk_version: Some(String::new()),
        file_version: None,
    }
}
...
fn empty_fields() {
    ...
    assert_eq!(eval("<bookclerk version>"), "");
    assert_eq!(eval("<year>"), "");
}
...
fn has_value_on_empty() {
    assert_eq!(eval("<has bookclerk version->empty-string<-has>"), "");
    assert_eq!(
        eval("<!has bookclerk version->empty-string<-has>"),
        "empty-string"
    );
    assert_eq!(
        eval("<is bookclerk version[=foobar]->empty-string<-is>"),
        ""
    );
    assert_eq!(
        eval("<is bookclerk version[=]->empty-string<-is>"),
        "empty-string"
    );
