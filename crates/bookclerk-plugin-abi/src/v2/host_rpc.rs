...
use super::host_roles::{AdapterTransaction, HostAdapterDatabaseSession};
use super::plugin_v2_capnp::adapter_database_session as adapter_database_session_capnp;
...
fn read_empty(reader: super::plugin_v2_capnp::empty_reply::Reader<'_>) -> Result<()> {
    match reader.which().map_err(from_capnp)? {
        ...
        super::plugin_v2_capnp::empty_reply::Err(err) => Err(read_error(err.map_err(from_capnp)?)),
    }
}

fn write_error(mut out: super::plugin_v2_capnp::plugin_error::Builder<'_>, err: &PluginError) {
