//! Receive descriptors the host passes over a side channel.
...
/// Descriptor the host leaves open for the plugin side channel.
...
/// Receive one descriptor from the host side channel.
#[cfg(unix)]
...
/// Resolve `/proc/self/fd/N` (Linux) or `/dev/fd/N` (macOS / BSD).
#[must_use]
