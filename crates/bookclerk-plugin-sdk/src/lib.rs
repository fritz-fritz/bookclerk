...
pub use bookclerk_plugin_abi::{
    ...
    AdapterDatabaseSession, AdapterExecuteRequest, ByteRange, Cancellation, ContentSource,
    ContentSourceClient, ContentSourceContext, CopyResult, Database, DatabaseAdapterConfig,
    DatabaseClient, DatabaseContext, DbBootstrap, DbCapabilities, DbColumn, DbPlanStatementKind,
    DbResultSelection, DbRow, DbTiming, DbType, DbValue, Destination, DestinationClient,
    DestinationContext, DestinationServer, DiagnoseResult, DomainEvent, EventResult, ExecuteReply,
    ExecuteRequest, GuestDatabase, HealthOk, Integration, IntegrationClient, IntegrationContext,
    IsolationReq, JobCheckpoint, JobHandler, JobHandlerContext, JobInvocation, JobInvocationLease,
    JobOutcome, ListOptions, ListPage, NeverCancel, ObjectInfo, ObjectMetadata, OidcClientTemplate,
    PluginClient, PluginDescribe, PluginError, PluginErrorCode, PluginRoot, PluginServer,
    ...
    TypedDbStatement, WorkerContext, WriteOptions, ABI_MAJOR, ABI_MINOR,
    DATABASE_ADAPTER_CONFIG_MEDIA_TYPE, ENVELOPE_VERSION, FEATURE_SCALAR_LIMITS,
