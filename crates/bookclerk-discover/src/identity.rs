    ...
    for entry in entries {
        if let Some(existing) = merged.iter_mut().find(|e| {
            ...
            if existing
                ...
                .trim()
                .is_empty()
                && !entry.cover_url.as_deref().unwrap_or("").trim().is_empty()
            {
            ...
            for ed in entry.store_editions {
                ...
                {
                    existing.store_editions.push(ed);
                }
            }
            for hint in entry.purchase_hints {
                if !existing
                    .purchase_hints
                    .iter()
                    .any(|h| h.source == hint.source && h.product_id == hint.product_id)
                {
                    existing.purchase_hints.push(hint);
                }
            }
