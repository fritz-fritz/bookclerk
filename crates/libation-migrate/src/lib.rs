//! Migrate data from classic (C#) Libation into libation-rs.
...
pub use discover::{discover_source, LibationSource};
...
/// Options for a full Libation Files directory import.
#[derive(Debug, Clone)]
...
/// Import classic Libation Files into a libation-rs data directory.
pub async fn migrate(opts: MigrateOptions) -> Result<MigrateSummary> {
    ...
    if let Some(db_path) = &source.library_db {
        ...
        summary.liberated = lib.liberated;
...
pub fn resolve_classic_files_dir(explicit: Option<&Path>) -> Option<std::path::PathBuf> {
    ...
    if let Ok(path) = std::env::var("LIBATION_CLASSIC_FILES") {
