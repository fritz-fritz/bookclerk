...
pub fn write_audiobook_chapters(path: &Path, chapters: &[(String, u64)]) -> Result<()> {
    ...
    let mdat = find_atom(&data, 0, data.len(), *b"mdat")
        .ok_or_else(|| DecryptError::Native("M4B missing mdat; cannot write chapters".into()))?;
    ...
    if movie_timescale == 0 {
        return Err(DecryptError::Native("mvhd timescale is zero".into()));
    }
...
fn set_audio_chap_ref(
    ...
    if kids.iter().all(|a| atom_type(moov, *a) != *b"tkhd") {
        return Err(DecryptError::Native("audio trak missing tkhd".into()));
    }
    ...
    let cleaned_tkhd_end = {
        ...
        let tk = iter_children(&cleaned, span)
            ...
            .ok_or_else(|| DecryptError::Native("cleaned audio trak missing tkhd".into()))?;
        tk.offset + tk.size
    };
