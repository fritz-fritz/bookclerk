// Historical incremental DDL kept for V27 FK-rebuild tests.
// These fragments are not a live upgrade path.

const LEGACY_GREENFIELD_SQLITE: &str = r#"
...
// (`IF NOT EXISTS` keeps the double apply safe).
...
/// Durable atomic receipts (idempotent replay after a lost response).
...
/// Bookkeeping version for the historical V27 FK-safe rebuild (not a live plan step).
#[must_use]
