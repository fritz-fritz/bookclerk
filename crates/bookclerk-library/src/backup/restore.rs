...
use super::util::{
    backup_assert_restore_constraints, backup_drop_user_relations, backup_import_identity,
    backup_list_user_relations, backup_prepare_unit_restore, exec_bound, ident_ok, table_exists,
};
...
pub async fn restore_backup_unit(
    ...
    let extra_drop = if kind == CanonicalRestoreKind::PluginBinding {
        match plugin_canonical_schema_from_ddl_catalog(db).await {
            ...
            Err(err) => {
                ...
                {
                    if binding_has_user_tables(db, opts.adapter.as_ref()).await? {
    ...
    // The adapter defers checks on the restore transaction, then asserts constraints
    ...
    backup_prepare_unit_restore(&txn, opts.adapter.as_ref()).await?;
    ...
    {
        Ok(()) => {
            if let Err(err) = backup_assert_restore_constraints(&txn, opts.adapter.as_ref()).await {
...
async fn drop_owned_schema<C>(
    ...
    adapter: Option<&bookclerk_plugin_abi::SharedAdapterBackupOps>,
) -> Result<()>
where
    C: ConnectionTrait,
{
    ...
    backup_drop_user_relations(conn, adapter, &names).await
}
...
/// Restore identity high-water values using adapter catalogs.
    ...
    C: ConnectionTrait,
{
    ...
    for (table, hw) in identity {
        ...
        rows.push(DbIdentityHighWater {
            table: table.clone(),
            last: hw.last,
        });
    }
    backup_import_identity(conn, opts.adapter.as_ref(), &rows).await
}
...
/// True when the binding has any non-engine user table (catalog missing + tables
/// means restore would merge).
async fn binding_has_user_tables(
    db: &DatabaseConnection,
    adapter: Option<&bookclerk_plugin_abi::SharedAdapterBackupOps>,
) -> Result<bool> {
    let names = backup_list_user_relations(db, adapter).await?;
    Ok(!names.is_empty())
}
